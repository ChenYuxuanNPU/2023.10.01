...
#这里用来给数据大屏加不同学段的统计信息
stat_for_pyecharts_major_discipline_and_high_school = {}
stat_for_pyecharts_major_discipline_and_vocational_school = {}
stat_for_pyecharts_major_discipline_and_middle_school = {}
stat_for_pyecharts_major_discipline_and_primary_school = {}

stat_for_pyecharts_current_age_and_high_school = {}
stat_for_pyecharts_current_age_and_vocational_school = {}
stat_for_pyecharts_current_age_and_middle_school = {}
stat_for_pyecharts_current_age_and_primary_school = {}
stat_for_pyecharts_current_age_and_kindergarten = {}

stat_for_pyecharts_educational_background_and_high_school = {}
stat_for_pyecharts_educational_background_and_vocational_school = {}
stat_for_pyecharts_educational_background_and_middle_school = {}
stat_for_pyecharts_educational_background_and_primary_school = {}
stat_for_pyecharts_educational_background_and_kindergarten = {}


#用来设置文件名
current_time = datetime.datetime.now().strftime('%Y-%m-%d')
save_path = r".\output\search_by_district" + '\\' + current_time

#用来设置排序
educational_background_order = {'高中及以下':1,'高中':2,'中专':3,"大学专科":4,"大学本科":5,"硕士研究生":6,"博士研究生":7,None:7}
highest_title_order = {'未取得职称':1,'三级教师':2,'二级教师':3,'一级教师':4,'高级教师':5,'正高级教师':6,'初级职称（非中小学系列）':7,'中级职称（非中小学系列）':8,'高级职称（非中小学系列）':9,None:10}
...
#这里对年龄进行统一统计，list_temp的格式为[[个数,年龄],[个数,年龄]]
def current_age_classification(list_temp):
...
def figure_for_educational_background_00():
    ...
    try:
        c.execute("select count(*),educational_background_highest from " + table_name + " group by educational_background_highest order by count(*) desc")
...
def figure_for_highest_title_00():
    ...
    try:
        c.execute("select count(*),highest_title from " + table_name + " group by highest_title order by count(*) desc")
    ...
    for single_data in result_for_highest_title:
        if single_data[1] is not None and single_data[1] != "无":
...
def log_for_current_age(label,data,avg_age):
    ...
    print("白云区公办在编教师平均年龄为" + str(avg_age) + "岁",file=file)
...
def figure_for_current_age_00():
    ...
    try:
        c.execute("select count(*),current_age from " + table_name + " group by current_age order by current_age asc")
    ...
    print("白云区公办在编教师平均年龄为" + str(avg_age) + "岁")
    ###
...
def log_for_major_discipline(label,data):
    ...
    for i in range(0, len(data)):
        print("主教" + str(label[i]) + "的教师:" + str(data[i]) + "人，占比" + str(round(100 * data[i] / generation_of_input_data.result.__len__(), 1)) + "%", file=file)
...
def figure_for_major_discipline_00():
    ...
    try:
        c.execute("select count(*),major_discipline from " + table_name + " where major_discipline is not null and major_discipline not like '%无%' group by major_discipline order by count(*) desc limit 20")
...
def log_for_school_title(label,data):
    ...
    for i in range(0, len(data)):
        print("毕业于" + str(label[i]) + "的教师:" + str(data[i]) + "人", file=file)
...
def figure_for_school_title_00():
    ...
    try:
        c.execute("select graduate_school_id from " + table_name + " where educational_background = '大学本科' or educational_background = '硕士研究生' or educational_background = '博士研究生'")
    ...
    for word in result_for_school_title:
        ...
        if(str(word[0]) not in verification_for_school_title.code_of_211):
    ...
    print([(counts_for_others,"其他院校"),(counts_for_211,"211院校"),(counts_for_985,"985院校"),(counts_for_affiliate,"部属师范院校")])
    ...
    for i in range(0,len(data_for_school_title)):
        if(data_for_school_title[i] == 0):
            print("白云区不存在毕业院校为" + str(label_for_school_title[i]) + "的教师")
            temp.append(i)
...
def figure_for_current_administrative_position_00():
    ...
    try:
        c.execute(
            "select count(*),current_administrative_position from " + table_name + " group by current_administrative_position order by case current_administrative_position when '无' then 1 when '中层副职' then 2 when '中层正职' then 3 when '副校级' then 4 when '正校级' then 5 when '党组织书记兼校长' then 6 when '党组织书记' then 7 else 8 end")
    ...
    log_for_current_administrative_position(label=label_for_current_administrative_position,data=data_for_current_administrative_position)
...
def figure_for_cadre_teacher_00():
    ...
    try:
        c.execute(
            "select count(*),cadre_teacher from " + table_name + " group by cadre_teacher order by case cadre_teacher when '无' then 1 when '其他' then 2 when '白云区骨干教师' then 3 when '广州市骨干教师' then 4 when '广东省骨干教师' then 5 else 6 end")
...
def figure_for_title_01_00():
    ...
    try:
        c.execute("select count(*) from " + table_name + " where title_01 != '无'")
    ...
    print("三名工作室主持人数：" + str(result_for_title_01[0][0]))
    #输出三名文档
    print("三名工作室主持人数：" + str(result_for_title_01[0][0]) + "，占比" + str(round(100 * result_for_title_01[0][0] / generation_of_input_data.result.__len__(),1)) + "%",file=file)
    print("",file=file)

    label_for_title_01 = ["三名工作室主持人","无"]
    data_for_title_01 = [result_for_title_01[0][0],len(generation_of_input_data.result)-result_for_title_01[0][0]]
...
def log_for_area_of_supporting_education(label,data):
    ...
    for i in range(0, len(data)):
        ...
        if(label[i] != "无"):
            print("在" + str(label[i]) + "支教的教师:" + str(data[i]) + "人，占比" + str(round(100 * data[i] / sum(data), 1)) + "%", file=file)

        else:
            print("无支教经历的教师：" + str(data[i]) + "人，占比" + str(round(100 * data[i] / sum(data), 1)) + "%", file=file)
...
def figure_for_area_of_supporting_education_00():
    ...
    try:
        c.execute(
            "select count(*),area_of_supporting_education from " + table_name + " group by area_of_supporting_education order by case area_of_supporting_education when '片内' then 1 when '区内' then 2 when '外市' then 3 when '外省' then 4 when '无' then 5 else 6 end")
    ...
    log_for_area_of_supporting_education(label=label_for_area_of_supporting_education,data=data_for_area_of_supporting_education)
...
def log_for_different_period(label,data):
    ...
    for i in range(0,len(label) - 1 ):
        print(str(label[i]) + "教师人数：" + str(data[i]) + "，占比" + str(round(100 * data[i] / sum(data), 1)) + "%", file=file)
...
def figure_for_different_period_00():
    ...
    try:
        c.execute("select period,major_discipline,current_age,educational_background_highest from " + table_name)
...
def figure_for_different_period_AND_major_discipline_00():
    ...
    global stat_for_pyecharts_major_discipline_and_high_school
    global stat_for_pyecharts_major_discipline_and_middle_school
    global stat_for_pyecharts_major_discipline_and_vocational_school
    global stat_for_pyecharts_major_discipline_and_primary_school
    ...
    for single_data in result_for_different_period:
        if(single_data[0] != "其他" and single_data[1] != "无"):
    ...
    #一边用来画图一边用来传数据大屏需要的信息
    stat_for_pyecharts_major_discipline_and_high_school = stat_for_high_school
    draw_figure_for_major_discipline_AND_high_school(stat_for_high_school)
    gc.collect()

    stat_for_pyecharts_major_discipline_and_middle_school = stat_for_middle_school
    draw_figure_for_major_discipline_AND_middle_school(stat_for_middle_school)
    gc.collect()

    stat_for_pyecharts_major_discipline_and_vocational_school = stat_for_vocational_school
    draw_figure_for_major_discipline_AND_vocational_school(stat_for_vocational_school)
    gc.collect()

    stat_for_pyecharts_major_discipline_and_primary_school = stat_for_primary_school
...
def figure_for_different_period_AND_current_age_00():
    ...
    global stat_for_pyecharts_current_age_and_high_school
    global stat_for_pyecharts_current_age_and_middle_school
    global stat_for_pyecharts_current_age_and_vocational_school
    global stat_for_pyecharts_current_age_and_primary_school
    global stat_for_pyecharts_current_age_and_kindergarten
    ...
    #这里同时准备数据大屏和本地画图
    stat_for_pyecharts_current_age_and_high_school = stat_for_high_school
    draw_figure_for_current_age_AND_high_school(stat_for_high_school)
    gc.collect()

    stat_for_pyecharts_current_age_and_middle_school = stat_for_middle_school
    draw_figure_for_current_age_AND_middle_school(stat_for_middle_school)
    gc.collect()

    stat_for_pyecharts_current_age_and_vocational_school = stat_for_vocational_school
    draw_figure_for_current_age_AND_vocational_school(stat_for_vocational_school)
    gc.collect()

    stat_for_pyecharts_current_age_and_primary_school = stat_for_primary_school
    draw_figure_for_current_age_AND_primary_school(stat_for_primary_school)
    gc.collect()

    stat_for_pyecharts_current_age_and_kindergarten = stat_for_kindergarten
...
def figure_for_different_period_AND_educational_background_00():
    ...
    global stat_for_pyecharts_educational_background_and_high_school
    global stat_for_pyecharts_educational_background_and_middle_school
    global stat_for_pyecharts_educational_background_and_vocational_school
    global stat_for_pyecharts_educational_background_and_primary_school
    global stat_for_pyecharts_educational_background_and_kindergarten
    ...
    #同时准备数据大屏和本地图表
    stat_for_pyecharts_educational_background_and_high_school = stat_for_high_school
    draw_figure_for_educational_background_AND_high_school(stat_for_high_school)
    gc.collect()

    stat_for_pyecharts_educational_background_and_middle_school = stat_for_middle_school
    draw_figure_for_educational_background_AND_middle_school(stat_for_middle_school)
    gc.collect()

    stat_for_pyecharts_educational_background_and_vocational_school = stat_for_vocational_school
    draw_figure_for_educational_background_AND_vocational_school(stat_for_vocational_school)
    gc.collect()

    stat_for_pyecharts_educational_background_and_primary_school = stat_for_primary_school
    draw_figure_for_educational_background_AND_primary_school(stat_for_primary_school)
    gc.collect()

    stat_for_pyecharts_educational_background_and_kindergarten = stat_for_kindergarten
...
def draw_pyecharts_for_district():
    ...
    label_for_different_period = []
    data_for_different_period = []
    list_temp = []


    pyechart_for_educational_background = (
        Pie(init_opts=opts.InitOpts(theme=ThemeType.LIGHT))
        .add("", [list(z) for z in zip(label_for_educational_background, data_for_educational_background)],center=["50%", "60%"],radius="65%")
        .set_global_opts(title_opts=opts.TitleOpts(title="最高学历"),legend_opts=opts.LegendOpts(pos_left='15%'))
    ...
    label_for_highest_title_updated = ['未取得职称', '三级教师', '二级教师', '一级教师', '高级教师', '正高级教师', '其他职称（非教师）']
    ...
    for i in range(0,len(label_for_highest_title)):
        for k in range(0,len(label_for_highest_title_updated)):
            ...
            elif(label_for_highest_title[i] in ['初级职称（非中小学系列）', '中级职称（非中小学系列）', '高级职称（非中小学系列）']):
    ...
    #print(label_for_highest_title_updated)
    #print(data_for_highest_title_updated)

    pyechart_for_highest_title = (
        Pie(init_opts=opts.InitOpts(theme=ThemeType.WESTEROS))
        .add("", [list(z) for z in zip(label_for_highest_title_updated, data_for_highest_title_updated)],center=["50%", "60%"],radius="65%")
    ...
    pyechart_for_current_age = (
        Pie(init_opts=opts.InitOpts(theme=ThemeType.WESTEROS))
        .add("", [list(z) for z in zip(label_for_current_age, data_for_current_age)],center=["50%", "60%"],radius="65%")
    ...
    #print("行政职务：")
    #print(label_for_current_administrative_position)
    #print(data_for_current_administrative_position)

    #洪老师说要把后面三项都放到正校级里
    label_for_current_administrative_position_updated = label_for_current_administrative_position[0:5]
    data_for_current_administrative_position1_updated = [data_for_current_administrative_position[0],data_for_current_administrative_position[1],data_for_current_administrative_position[2],data_for_current_administrative_position[3],sum(data_for_current_administrative_position[i] for i in range(4,len(data_for_current_administrative_position)))]
    ...
    pyechart_for_current_administrative_position = (
        Pie(init_opts=opts.InitOpts(theme=ThemeType.WESTEROS))
        .add("", [list(z) for z in zip(label_for_current_administrative_position_updated, data_for_current_administrative_position1_updated)],center=["50%", "60%"],radius="65%")
    ...
    #这里要重新调整顺序
    print(label_for_school_title)
    print(data_for_school_title)

    label_for_school_title_updated = ['985院校' , '部属师范院校' , '211院校' , '其他院校']
    data_for_school_title_updated = [data_for_school_title[2],data_for_school_title[3],data_for_school_title[1],data_for_school_title[0]]

    pyechart_for_school_title = (
        Bar(init_opts=opts.InitOpts(theme=ThemeType.LIGHT))
        .add_xaxis(label_for_school_title_updated)
        .add_yaxis("总人数",data_for_school_title_updated)
        .set_global_opts(title_opts=opts.TitleOpts(title="毕业院校级别（本科及以上）"))
    )
    ...
    #print(label_for_cadre_teacher_updated)
    #print(data_for_cadre_teacher_updated)


    pyechart_for_cadre_teacher = (
        Pie()
        .add("", [list(z) for z in zip(label_for_cadre_teacher_updated, data_for_cadre_teacher_updated)],center=["50%", "60%"],radius="65%")
        .set_global_opts(title_opts=opts.TitleOpts(title="骨干教师"),legend_opts=opts.LegendOpts(pos_left='15%'))
        .set_series_opts(label_opts=opts.LabelOpts(formatter="{b}: {d}%"),tooltip_opts=opts.TooltipOpts(trigger="item", formatter="{b}: {d}%"))
        #.set_series_opts(label_opts=opts.LabelOpts(formatter="{b}: {c},占比{d}%"),tooltip_opts=opts.TooltipOpts(trigger="item", formatter="{b}: {c} ({d}%)"))
    )
    ...
    pyechart_for_area_of_supporting_education = (
        Pie()
        .add("", [list(z) for z in zip(label_for_area_of_supporting_education, data_for_area_of_supporting_education)],center=["50%", "60%"],radius="65%")
    ...
    for major_discipline,number in stat_for_pyecharts_major_discipline_and_high_school.items():
        list_temp.append([number,major_discipline])

    list_temp.sort(key = lambda x1: x1[0], reverse=True)

    for single_data in list_temp:
        label_for_different_period.append(single_data[1])
        data_for_different_period.append(single_data[0])
        if(len(label_for_different_period) > 20):
            break

    label_for_different_period = list(reversed(label_for_different_period))
    data_for_different_period = list(reversed(data_for_different_period))

    pyechart_for_major_discipline_and_high_school = (
        Bar(init_opts=opts.InitOpts(theme=ThemeType.LIGHT))
        .add_xaxis(label_for_different_period)
        .add_yaxis("总人数",data_for_different_period)
        .reversal_axis()
        .set_series_opts(label_opts=opts.LabelOpts(position="right"))
        .set_global_opts(title_opts=opts.TitleOpts(title="主教学科"))
    )
    pyechart_for_major_discipline_and_high_school.render("pyecharts\pyechart_for_major_discipline_and_high_school.html")
    ...
    for major_discipline,number in stat_for_pyecharts_major_discipline_and_middle_school.items():
        list_temp.append([number,major_discipline])
    ...
    pyechart_for_major_discipline_and_middle_school = (
        Bar(init_opts=opts.InitOpts(theme=ThemeType.LIGHT))
        .add_xaxis(label_for_different_period)
        .add_yaxis("总人数",data_for_different_period)
        .reversal_axis()
    ...
    pyechart_for_major_discipline_and_middle_school.render("pyecharts\pyechart_for_major_discipline_and_middle_school.html")
    pyechart_for_major_discipline_and_middle_school.render_notebook()
    ...
    for major_discipline,number in stat_for_pyecharts_major_discipline_and_vocational_school.items():
        list_temp.append([number,major_discipline])
    ...
    pyechart_for_major_discipline_and_vocational_school = (
        Bar(init_opts=opts.InitOpts(theme=ThemeType.LIGHT))
        .add_xaxis(label_for_different_period)
        .add_yaxis("总人数",data_for_different_period)
        .reversal_axis()
    ...
    pyechart_for_major_discipline_and_vocational_school.render("pyecharts\pyechart_for_major_discipline_and_vocational_school.html")
    pyechart_for_major_discipline_and_vocational_school.render_notebook()
    ...
    for major_discipline,number in stat_for_pyecharts_major_discipline_and_primary_school.items():
        list_temp.append([number,major_discipline])
    ...
    pyechart_for_major_discipline_and_primary_school = (
        Bar(init_opts=opts.InitOpts(theme=ThemeType.LIGHT))
        .add_xaxis(label_for_different_period)
        .add_yaxis("总人数",data_for_different_period)
        .reversal_axis()
    ...
    pyechart_for_major_discipline_and_primary_school.render("pyecharts\pyechart_for_major_discipline_and_primary_school.html")
    pyechart_for_major_discipline_and_primary_school.render_notebook()
    ...
    for educational_background,number in stat_for_pyecharts_educational_background_and_high_school.items():
        list_temp.append([number,educational_background])

    list_temp = sorted(list_temp,key=lambda x:educational_background_order[x[1]])

    for single_data in list_temp:
        data_for_different_period.append(single_data[0])
        label_for_different_period.append(single_data[1])

    pyechart_for_educational_background_and_high_school = (
        Pie(init_opts=opts.InitOpts(theme=ThemeType.WALDEN))
        .add("", [list(z) for z in zip(label_for_different_period, data_for_different_period)],center=["50%", "60%"],radius="65%")
        .set_global_opts(title_opts=opts.TitleOpts(title="最高学历"),legend_opts=opts.LegendOpts(pos_left='15%'))
        .set_series_opts(label_opts=opts.LabelOpts(formatter="{b}: {d}%"),tooltip_opts=opts.TooltipOpts(trigger="item", formatter="{b}: {d}%"))
        #.set_series_opts(label_opts=opts.LabelOpts(formatter="{b}: {c},占比{d}%"),tooltip_opts=opts.TooltipOpts(trigger="item", formatter="{b}: {c} ({d}%)"))
    )
    pyechart_for_educational_background_and_high_school.render("pyecharts\pyechart_for_educational_background_and_high_school.html")
    pyechart_for_educational_background_and_high_school.render_notebook()
    ...
    for educational_background,number in stat_for_pyecharts_educational_background_and_middle_school.items():
        list_temp.append([number,educational_background])
    ...
    pyechart_for_educational_background_and_middle_school = (
        Pie(init_opts=opts.InitOpts(theme=ThemeType.WALDEN))
        .add("", [list(z) for z in zip(label_for_different_period, data_for_different_period)],center=["50%", "60%"],radius="65%")
        .set_global_opts(title_opts=opts.TitleOpts(title="最高学历"),legend_opts=opts.LegendOpts(pos_left='15%'))
        .set_series_opts(label_opts=opts.LabelOpts(formatter="{b}: {d}%"),tooltip_opts=opts.TooltipOpts(trigger="item", formatter="{b}: {d}%"))
    ...
    pyechart_for_educational_background_and_middle_school.render("pyecharts\pyechart_for_educational_background_and_middle_school.html")
    pyechart_for_educational_background_and_middle_school.render_notebook()
    ...
    for educational_background,number in stat_for_pyecharts_educational_background_and_vocational_school.items():
        list_temp.append([number,educational_background])
    ...
    pyechart_for_educational_background_and_vocational_school = (
        Pie(init_opts=opts.InitOpts(theme=ThemeType.WALDEN))
        .add("", [list(z) for z in zip(label_for_different_period, data_for_different_period)],center=["50%", "60%"],radius="65%")
        .set_global_opts(title_opts=opts.TitleOpts(title="最高学历"),legend_opts=opts.LegendOpts(pos_left='15%'))
        .set_series_opts(label_opts=opts.LabelOpts(formatter="{b}: {d}%"),tooltip_opts=opts.TooltipOpts(trigger="item", formatter="{b}: {d}%"))
    ...
    pyechart_for_educational_background_and_vocational_school.render("pyecharts\pyechart_for_educational_background_and_vocational_school.html")
    pyechart_for_educational_background_and_vocational_school.render_notebook()
    ...
    for educational_background,number in stat_for_pyecharts_educational_background_and_primary_school.items():
        list_temp.append([number,educational_background])
    ...
    pyechart_for_educational_background_and_primary_school = (
        Pie(init_opts=opts.InitOpts(theme=ThemeType.WALDEN))
        .add("", [list(z) for z in zip(label_for_different_period, data_for_different_period)],center=["50%", "60%"],radius="65%")
        .set_global_opts(title_opts=opts.TitleOpts(title="最高学历"),legend_opts=opts.LegendOpts(pos_left='15%'))
        .set_series_opts(label_opts=opts.LabelOpts(formatter="{b}: {d}%"),tooltip_opts=opts.TooltipOpts(trigger="item", formatter="{b}: {d}%"))
    ...
    pyechart_for_educational_background_and_primary_school.render("pyecharts\pyechart_for_educational_background_and_primary_school.html")
    pyechart_for_educational_background_and_primary_school.render_notebook()
    ...
    for educational_background,number in stat_for_pyecharts_educational_background_and_kindergarten.items():
        list_temp.append([number,educational_background])
    ...
    pyechart_for_educational_background_and_kindergarten = (
        Pie(init_opts=opts.InitOpts(theme=ThemeType.WALDEN))
        .add("", [list(z) for z in zip(label_for_different_period, data_for_different_period)],center=["50%", "60%"],radius="65%")
        .set_global_opts(title_opts=opts.TitleOpts(title="最高学历"),legend_opts=opts.LegendOpts(pos_left='15%'))
        .set_series_opts(label_opts=opts.LabelOpts(formatter="{b}: {d}%"),tooltip_opts=opts.TooltipOpts(trigger="item", formatter="{b}: {d}%"))
    ...
    pyechart_for_educational_background_and_kindergarten.render("pyecharts\pyechart_for_educational_background_and_kindergarten.html")
    pyechart_for_educational_background_and_kindergarten.render_notebook()
    ...
    for current_age, number in stat_for_pyecharts_current_age_and_high_school.items():
        list_temp.append([number, current_age])

    data_for_different_period,label_for_different_period = current_age_classification(list_temp=list_temp)

    pyechart_for_current_age_and_high_school = (
        Pie(init_opts=opts.InitOpts(theme=ThemeType.WESTEROS))
        .add("", [list(z) for z in zip(label_for_different_period, data_for_different_period)],center=["50%", "60%"],radius="65%")
        .set_global_opts(title_opts=opts.TitleOpts(title="年龄分布"),legend_opts=opts.LegendOpts(pos_left='15%'))
        .set_series_opts(label_opts=opts.LabelOpts(formatter="{b}: {d}%"),tooltip_opts=opts.TooltipOpts(trigger="item", formatter="{b}: {d}%"))
        #.set_series_opts(label_opts=opts.LabelOpts(formatter="{b}: {c},占比{d}%"),tooltip_opts=opts.TooltipOpts(trigger="item", formatter="{b}: {c} ({d}%)"))
    )
    pyechart_for_current_age_and_high_school.render("pyecharts\pyechart_for_current_age_and_high_school.html")
    pyechart_for_current_age_and_high_school.render_notebook()
    ...
    for current_age, number in stat_for_pyecharts_current_age_and_middle_school.items():
        list_temp.append([number, current_age])

    data_for_different_period,label_for_different_period = current_age_classification(list_temp=list_temp)

    pyechart_for_current_age_and_middle_school = (
        Pie(init_opts=opts.InitOpts(theme=ThemeType.WESTEROS))
        .add("", [list(z) for z in zip(label_for_different_period, data_for_different_period)],center=["50%", "60%"],radius="65%")
        .set_global_opts(title_opts=opts.TitleOpts(title="年龄分布"),legend_opts=opts.LegendOpts(pos_left='15%'))
        .set_series_opts(label_opts=opts.LabelOpts(formatter="{b}: {d}%"),tooltip_opts=opts.TooltipOpts(trigger="item", formatter="{b}: {d}%"))
    ...
    pyechart_for_current_age_and_middle_school.render("pyecharts\pyechart_for_current_age_and_middle_school.html")
    pyechart_for_current_age_and_middle_school.render_notebook()
    ...
    for current_age, number in stat_for_pyecharts_current_age_and_vocational_school.items():
        list_temp.append([number, current_age])

    data_for_different_period,label_for_different_period = current_age_classification(list_temp=list_temp)

    pyechart_for_current_age_and_vocational_school = (
        Pie(init_opts=opts.InitOpts(theme=ThemeType.WESTEROS))
        .add("", [list(z) for z in zip(label_for_different_period, data_for_different_period)],center=["50%", "60%"],radius="65%")
        .set_global_opts(title_opts=opts.TitleOpts(title="年龄分布"),legend_opts=opts.LegendOpts(pos_left='15%'))
        .set_series_opts(label_opts=opts.LabelOpts(formatter="{b}: {d}%"),tooltip_opts=opts.TooltipOpts(trigger="item", formatter="{b}: {d}%"))
    ...
    pyechart_for_current_age_and_vocational_school.render("pyecharts\pyechart_for_current_age_and_vocational_school.html")
    pyechart_for_current_age_and_vocational_school.render_notebook()
    ...
    for current_age, number in stat_for_pyecharts_current_age_and_primary_school.items():
        list_temp.append([number, current_age])

    data_for_different_period,label_for_different_period = current_age_classification(list_temp=list_temp)

    pyechart_for_current_age_and_primary_school = (
        Pie(init_opts=opts.InitOpts(theme=ThemeType.WESTEROS))
        .add("", [list(z) for z in zip(label_for_different_period, data_for_different_period)],center=["50%", "60%"],radius="65%")
        .set_global_opts(title_opts=opts.TitleOpts(title="年龄分布"),legend_opts=opts.LegendOpts(pos_left='15%'))
        # .set_series_opts(label_opts=opts.LabelOpts(formatter="{b}: {d}%"),tooltip_opts=opts.TooltipOpts(trigger="item", formatter="{b}: {d}%"))
    ...
    pyechart_for_current_age_and_primary_school.render("pyecharts\pyechart_for_current_age_and_primary_school.html")
    pyechart_for_current_age_and_primary_school.render_notebook()
    ...
    for current_age, number in stat_for_pyecharts_current_age_and_kindergarten.items():
        list_temp.append([number, current_age])

    data_for_different_period,label_for_different_period = current_age_classification(list_temp=list_temp)

    pyechart_for_current_age_and_kindergarten = (
        Pie(init_opts=opts.InitOpts(theme=ThemeType.WESTEROS))
        .add("", [list(z) for z in zip(label_for_different_period, data_for_different_period)],center=["50%", "60%"],radius="65%")
        .set_global_opts(title_opts=opts.TitleOpts(title="年龄分布"),legend_opts=opts.LegendOpts(pos_left='15%'))
        .set_series_opts(label_opts=opts.LabelOpts(formatter="{b}: {d}%"),tooltip_opts=opts.TooltipOpts(trigger="item", formatter="{b}: {d}%"))
    ...
    pyechart_for_current_age_and_kindergarten.render("pyecharts\pyechart_for_current_age_and_kindergarten.html")
    pyechart_for_current_age_and_kindergarten.render_notebook()
    ...
    big_title = (
        Pie() # 不画图，只显示一个标题，用来构成大屏的标题
            .set_global_opts(
            title_opts=opts.TitleOpts(title="白云区教师信息可视化大屏",title_textstyle_opts=opts.TextStyleOpts(font_size=32),pos_top=10),
    ...
    title_high_school = (
        Pie() # 不画图，只显示一个标题，用来构成大屏的标题
            .set_global_opts(
            title_opts=opts.TitleOpts(title="高中信息统计",title_textstyle_opts=opts.TextStyleOpts(font_size=32),pos_top=10),
            legend_opts=opts.LegendOpts(is_show=False)
            )
    )
    title_high_school.render_notebook()

    title_middle_school = (
        Pie() # 不画图，只显示一个标题，用来构成大屏的标题
            .set_global_opts(
            title_opts=opts.TitleOpts(title="初中信息统计",title_textstyle_opts=opts.TextStyleOpts(font_size=32),pos_top=10),
            legend_opts=opts.LegendOpts(is_show=False)
            )
    )
    title_middle_school.render_notebook()

    title_vocational_school = (
        Pie() # 不画图，只显示一个标题，用来构成大屏的标题
            .set_global_opts(
            title_opts=opts.TitleOpts(title="职中信息统计",title_textstyle_opts=opts.TextStyleOpts(font_size=32),pos_top=10),
            legend_opts=opts.LegendOpts(is_show=False)
            )
    )
    title_vocational_school.render_notebook()

    title_primary_school = (
        Pie() # 不画图，只显示一个标题，用来构成大屏的标题
            .set_global_opts(
            title_opts=opts.TitleOpts(title="小学信息统计",title_textstyle_opts=opts.TextStyleOpts(font_size=32),pos_top=10),
            legend_opts=opts.LegendOpts(is_show=False)
            )
    )
    title_primary_school.render_notebook()

    title_kindergarten = (
        Pie() # 不画图，只显示一个标题，用来构成大屏的标题
            .set_global_opts(
            title_opts=opts.TitleOpts(title="幼儿园信息统计",title_textstyle_opts=opts.TextStyleOpts(font_size=32),pos_top=10),
            legend_opts=opts.LegendOpts(is_show=False)
            )
    )
    title_kindergarten.render_notebook()
    ...
    with open(save_path + "\\page.html", "r+", encoding='utf-8') as html:
        ...
        #全区信息的样式
        divs[0]["style"] = "width:50%;height:30%;position:absolute;top:0%;left:38%;border-style:dashed;border-color:#89641;border-width:0px;"
        divs[1]["style"] = "width:47%;height:90%;position:absolute;top:10%;left:3%;border-style:solid;border-color:#ffffff;border-width:2px;"
        divs[2]["style"] = "width:47%;height:45%;position:absolute;top:10%;left:50%;border-style:solid;border-color:#ffffff;border-width:2px;"
        divs[3]["style"] = "width:47%;height:45%;position:absolute;top:55%;left:50%;border-style:solid;border-color:#ffffff;border-width:2px;"
        divs[4]["style"] = "width:47%;height:45%;position:absolute;top:100%;left:3%;border-style:solid;border-color:#ffffff;border-width:2px;"
        divs[5]["style"] = "width:47%;height:45%;position:absolute;top:100%;left:50%;border-style:solid;border-color:#ffffff;border-width:2px;"
        divs[6]["style"] = "width:47%;height:45%;position:absolute;top:150%;left:3%;border-style:solid;border-color:#ffffff;border-width:2px;"
        divs[7]["style"] = "width:47%;height:45%;position:absolute;top:150%;left:50%;border-style:solid;border-color:#ffffff;border-width:2px;"
        divs[8]["style"] = "width:47%;height:45%;position:absolute;top:200%;left:3%;border-style:solid;border-color:#ffffff;border-width:2px;"
        divs[9]["style"] = "width:47%;height:45%;position:absolute;top:200%;left:50%;border-style:solid;border-color:#ffffff;border-width:2px;"

        #高中信息的样式
        divs[10]["style"] = "width:50%;height:30%;position:absolute;top:250%;left:43%;border-style:dashed;border-color:#89641;border-width:0px;"
        divs[11]["style"] = "width:47%;height:90%;position:absolute;top:260%;left:3%;border-style:solid;border-color:#ffffff;border-width:2px;"
        divs[12]["style"] = "width:47%;height:45%;position:absolute;top:260%;left:50%;border-style:solid;border-color:#ffffff;border-width:2px;"
        divs[13]["style"] = "width:47%;height:45%;position:absolute;top:305%;left:50%;border-style:solid;border-color:#ffffff;border-width:2px;"

        #初中信息的样式
        divs[14]["style"] = "width:50%;height:30%;position:absolute;top:350%;left:43%;border-style:dashed;border-color:#89641;border-width:0px;"
        divs[15]["style"] = "width:47%;height:90%;position:absolute;top:360%;left:3%;border-style:solid;border-color:#ffffff;border-width:2px;"
        divs[16]["style"] = "width:47%;height:45%;position:absolute;top:360%;left:50%;border-style:solid;border-color:#ffffff;border-width:2px;"
        divs[17]["style"] = "width:47%;height:45%;position:absolute;top:405%;left:50%;border-style:solid;border-color:#ffffff;border-width:2px;"

        #职中信息的样式
        divs[18]["style"] = "width:50%;height:30%;position:absolute;top:450%;left:43%;border-style:dashed;border-color:#89641;border-width:0px;"
        divs[19]["style"] = "width:47%;height:90%;position:absolute;top:460%;left:3%;border-style:solid;border-color:#ffffff;border-width:2px;"
        divs[20]["style"] = "width:47%;height:45%;position:absolute;top:460%;left:50%;border-style:solid;border-color:#ffffff;border-width:2px;"
        divs[21]["style"] = "width:47%;height:45%;position:absolute;top:505%;left:50%;border-style:solid;border-color:#ffffff;border-width:2px;"

        #小学信息的样式
        divs[22]["style"] = "width:50%;height:30%;position:absolute;top:550%;left:43%;border-style:dashed;border-color:#89641;border-width:0px;"
        divs[23]["style"] = "width:47%;height:90%;position:absolute;top:560%;left:3%;border-style:solid;border-color:#ffffff;border-width:2px;"
        divs[24]["style"] = "width:47%;height:45%;position:absolute;top:560%;left:50%;border-style:solid;border-color:#ffffff;border-width:2px;"
        divs[25]["style"] = "width:47%;height:45%;position:absolute;top:605%;left:50%;border-style:solid;border-color:#ffffff;border-width:2px;"

        #幼儿园信息的样式
        divs[26]["style"] = "width:50%;height:30%;position:absolute;top:650%;left:43%;border-style:dashed;border-color:#89641;border-width:0px;"
        divs[27]["style"] = "width:47%;height:60%;position:absolute;top:660%;left:3%;border-style:solid;border-color:#ffffff;border-width:2px;"
        divs[28]["style"] = "width:47%;height:60%;position:absolute;top:660%;left:50%;border-style:solid;border-color:#ffffff;border-width:2px;"
