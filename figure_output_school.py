...
#设置学校列表
school_list = ["广州市白云区广州空港实验中学"]
school_name = "广州市白云区广州空港实验中学"
...
save_path = r".\output\search_by_school" + '\\' + current_time + '\\' +school_name
...
def figure_for_educational_background_00():
    ...
    try:
        c.execute("select count(*),educational_background_highest from " + table_name + " where school_name = '" + school_name + "' group by educational_background_highest order by count(*) desc")
...
def figure_for_highest_title_00():
    ...
    try:
        c.execute("select count(*),highest_title from " + table_name + " where school_name = '" + school_name + "' group by highest_title order by count(*) desc")
...
def log_for_current_age(label,data,avg_age):
    ...
    print(school_name + "公办在编教师平均年龄为" + str(avg_age) + "岁",file=file)
...
def figure_for_current_age_00():
    ...
    try:
        c.execute("select count(*),current_age from " + table_name + " where school_name = '" + school_name + "' group by current_age order by current_age asc")
    ...
    print(school_name + "公办在编教师平均年龄为" + str(avg_age) + "岁")
    ###
...
def log_for_major_discipline(label,data):
    ...
    for i in range(0, len(data)):
        print("主教" + str(label[i]) + "的教师:" + str(data[i]) + "人，占比" + str(round(100 * data[i] / sum(data_for_major_discipline),1)) + "%", file=file)
...
def figure_for_major_discipline_00():
    ...
    try:
        c.execute("select count(*),major_discipline from " + table_name + " where school_name = '" + school_name + "' and major_discipline is not null and major_discipline not like '%无%' group by major_discipline order by count(*) desc limit 20")
    ...
    ax.set(ylabel='人数', title=school_name + '教师主教学科统计', ylim=(0, axis_height))
    ax.bar_label(bar_container)
...
def log_for_school_title(label,data):
    ...
    for i in range(0, len(data)):
        if(sum(data) != 0):
            print("毕业于" + str(label[i]) + "的教师:" + str(data[i]) + "人，占比" + str(round(100 * data[i] / sum(data), 1)) + "%", file=file)
...
def figure_for_school_title_00():
    ...
    try:
        c.execute("select graduate_school_id from " + table_name + " where (educational_background = '大学本科' or educational_background = '硕士研究生' or educational_background = '博士研究生') and school_name = '" + school_name + "'")
        #"select graduate_school_id from " + table_name + " where area = '" + school_name + "'"
    ...
    for i in range(0,len(data_for_school_title)):
        if(data_for_school_title[i] == 0):
            print(school_name + "不存在毕业院校为" + str(label_for_school_title[i]) + "的教师")
            temp.append(i)
...
def figure_for_cadre_teacher_00():
    ...
    try:
        c.execute(
            "select count(*),cadre_teacher from " + table_name + " where school_name = '" + school_name + "' group by cadre_teacher order by case cadre_teacher when '无' then 1 when '其他' then 2 when '白云区骨干教师' then 3 when '广州市骨干教师' then 4 when '广东省骨干教师' then 5 else 6 end")
...
def figure_for_title_01_00():
    ...
    try:
        c.execute("select(select count(*) from " + table_name + " where school_name = '" + school_name + "'), (select count(*) from " + table_name + " where school_name = '" + school_name + "' and title_01 != '无')")
    ...
    #输出三名文档
    print("三名工作室主持人数：" + str(result_for_title_01[0][1]) + "，占比" + str(round(100 * result_for_title_01[0][1] / result_for_title_01[0][0],1)) + "%",file=file)
    print("",file=file)
...
def figure_for_area_of_supporting_education_00():
    ...
    try:
        c.execute(
            "select count(*),area_of_supporting_education from " + table_name + " where school_name = '" + school_name + "' group by area_of_supporting_education order by case area_of_supporting_education when '片内' then 1 when '区内' then 2 when '外市' then 3 when '外省' then 4 when '无' then 5 else 6 end")
...
def figure_for_current_administrative_position_00():
    ...
    try:
        c.execute(
            "select count(*),current_administrative_position from " + table_name + " where school_name = '" + school_name + "' group by current_administrative_position order by case current_administrative_position when '无' then 1 when '中层副职' then 2 when '中层正职' then 3 when '副校级' then 4 when '正校级' then 5 when '党组织书记兼校长' then 6 when '党组织书记' then 7 else 8 end")
...
def figure_for_different_period_00():
    ...
    try:
        c.execute("select grade_to_teach,major_discipline,current_age,educational_background from " + table_name + " where school_name = '" + school_name +  "'")
    ...
    for i in range(0, len(result_for_different_period)):
        # print(result[i][0])
        if ("高" in result_for_different_period[i][0]):
            data[0] = data[0] + 1
            result_for_different_period[i][0] = label[0]
        elif ("初" in result_for_different_period[i][0] or "九" in result_for_different_period[i][0] or "八" in result_for_different_period[i][0] or "七" in result_for_different_period[i][0] or "9" in
              result_for_different_period[i][0] or "8" in result_for_different_period[i][0] or "7" in result_for_different_period[i][0]):
            data[1] = data[1] + 1
            result_for_different_period[i][0] = label[1]
        elif ("职" in result_for_different_period[i][0]):
            data[2] = data[2] + 1
            result_for_different_period[i][0] = label[2]
        elif ("六" in result_for_different_period[i][0] or "五" in result_for_different_period[i][0] or "四" in result_for_different_period[i][0] or "三" in result_for_different_period[i][0] or "二" in
              result_for_different_period[i][0] or "一" in result_for_different_period[i][0] or "6" in result_for_different_period[i][0] or "5" in result_for_different_period[i][0] or "4" in result_for_different_period[i][
                  0] or "3" in result_for_different_period[i][0] or "2" in result_for_different_period[i][0] or "1" in result_for_different_period[i][0]):
            data[3] = data[3] + 1
            result_for_different_period[i][0] = label[3]
        elif ("大班" in result_for_different_period[i][0] or "中班" in result_for_different_period[i][0] or "小班" in result_for_different_period[i][0]):
            data[4] = data[4] + 1
            result_for_different_period[i][0] = label[4]
        elif ("其他" in result_for_different_period[i][0] or "无" in result_for_different_period[i][0]):
            data[5] = data[5] + 1
...
def update_folder():
    ...
    if(os.path.exists(r".\output\search_by_school" + '\\' + current_time)):
        pass
    else:
        os.mkdir(r".\output\search_by_school" + '\\' + current_time)
...
def draw_pyecharts_for_district():
    ...
    count_four_position = 0

    #这里出bug是因为将后几项融在一起，但是有的学校缺了中层正副职导致数组越界
    if(label_for_current_administrative_position.__len__() == 8):
        ...
        count_four_position = label_for_current_administrative_position.count("无") + label_for_current_administrative_position.count("中层正职") + label_for_current_administrative_position.count("中层副职") + label_for_current_administrative_position.count("副校级")
        count_principal = label_for_current_administrative_position.count("正校级") + label_for_current_administrative_position.count("党组织书记兼校长") + label_for_current_administrative_position.count("党组织书记兼校长") + label_for_current_administrative_position.count("党组织书记")

        if(count_principal == 0):
            label_for_current_administrative_position_updated = label_for_current_administrative_position
            data_for_current_administrative_position1_updated = data_for_current_administrative_position

        else:
            label_for_current_administrative_position_updated = label_for_current_administrative_position[0:count_four_position]
            label_for_current_administrative_position_updated.append("正校级")

            data_for_current_administrative_position1_updated = []

            for i in range(0,count_four_position):
                data_for_current_administrative_position1_updated.append(data_for_current_administrative_position[i])

            data_for_current_administrative_position1_updated.append(sum(data_for_current_administrative_position[i] for i in range(count_four_position,len(data_for_current_administrative_position))))
    ...
    big_title = (
        Pie() # 不画图，只显示一个标题，用来构成大屏的标题
            .set_global_opts(
            title_opts=opts.TitleOpts(title=school_name + "可视化大屏",title_textstyle_opts=opts.TextStyleOpts(font_size=32),text_align="Center",pos_top=10),
    ...
    page.render(r".\output\search_by_school" + '\\' + school_name + '数据大屏.html')

    with open( r".\output\search_by_school" + '\\' + school_name + "数据大屏.html", "r+", encoding='utf-8') as html:
        html_bf = BeautifulSoup(html, 'html.parser')
        divs = html_bf.select('.chart-container') # 根据css定位标签，选中图像的父节点标签
        divs[0]["style"] = "width:50%;height:30%;position:absolute;top:0%;left:33%;border-style:dashed;border-color:#89641;border-width:0px;"
